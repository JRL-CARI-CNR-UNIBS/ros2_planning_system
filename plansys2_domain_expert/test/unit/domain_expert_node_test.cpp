...
TEST(domain_expert, extend_domain)
{
  {
    auto test_node = rclcpp::Node::make_shared("extend_domain_test");
    auto domain_node = std::make_shared<plansys2::DomainExpertNode>();
    auto domain_client = std::make_shared<plansys2::DomainExpertClient>();

    std::string pkgpath =
      ament_index_cpp::get_package_share_directory("plansys2_domain_expert");

    domain_node->set_parameter({"model_file", pkgpath + "/pddl/domain_1_ok.pddl"});
    ...
    const auto domain_before = domain_client->getDomain();
...
(:durative-action greet
    ...
    const bool success = domain_client->extendDomain(extension);
    ASSERT_TRUE(success);
    ...
    const auto domain_after = domain_client->getDomain();

    ASSERT_NE(domain_before, domain_after);
    ASSERT_NE(domain_after.find("greet"), std::string::npos);
    ASSERT_NE(domain_after.find(":durative-action greet"), std::string::npos);
